<div class="headerName"> update Admin {{ $basics[0]->name_en }}'s profile </div>
...
<div class="form-group form_style">
    <form id="" action="/Panel/Admin/{{ $user->id }}"  class="form-group" method="Post">
        @method('PATCH')
        @csrf
            {{--{{ method_field('PUT') }}--}}
            ...
            <div class="form-group row">
                <label for="working_hours" class="col-2 col-form-label">Working Hours:</label>
                <div class="col-10">
                    <input type="text" class="form-control input-sm" name="working_hours" value="{{ $user->working_hours }}" required>
                </div>
            </div>
