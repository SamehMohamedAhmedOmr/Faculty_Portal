<h6 class="headerName"> Manage Courses </h6>
<form class="form-horizontal" method="GET" action="/Panel/Doctor/ManageCourse">
...
@if(isset($student) && count($student)>0)
    ...
    <div class="alert alert-secondary row">
        @if(Request::input('SemesterName')==$AvilableSemester)
            <div class="col-12" style="text-align: center;">{{ 'Upload New Material to Class Room' }}</div> <br><br>
             <form class="form col-12 row" action="/Panel/Doctor/UploadFile" style="margin-bottom: 5px;"  method="POST" enctype="multipart/form-data">
                 @csrf
                ...
                <div  style="margin-bottom: 10px;">
                    @foreach($AllMaterial as $material)
                        ...
                        <div class="d-inline-block" style="margin-bottom: 5px;margin-right: 5px;" title="{{$material->description}}">
                            ...
                            <form method="POST" style="display: inline-block;" action="/Panel/Doctor/RemoveFile">
                                @csrf
                                <input type="hidden" name="fileName" value="{{$material->file}}">
                                <input type="hidden" name="semesterID" value="{{Request::input('SemesterName')}}">
                                <input type="hidden" name="dateTime" value="{{$material->date}}">
                                <input type="hidden" name="subjectID" value="{{Request::input('courseID')}}">
                                <button type="submit" class="form-control btn-light"><i class="fas fa-times-circle"></i></button>
                            </form>
                        </div>
                    @endforeach
                </div>
    ...
    <table class="table table-bordered" style="position: relative;">
            ...
            @foreach($student as $stu)
                ...
                <tr class="setRow">
                    ...
                    <td><input style="text-align:center;" name="midterm" type="number" @if(isset($grades->semester_id) && $grades->semester_id != $AvilableSemester ) disabled @endif @if(isset($grades)) value="{{$grades->midterm}}" @else value="0"  @endif min="0" max="{{$grade_dist->midterm}}"></td>
                    @php
                        if($quiz==1)
                        {
                            echo "<td><input ".((isset($grades->semester_id) && $grades->semester_id != $AvilableSemester )?' disabled':'')." style=\"text-align:center;\" name='quiz' ".((isset($grades->quiz))?' value='.$grades->quiz.' ':' value="0"')." type=\"number\" min=\"0\" max=\"".$grade_dist->quiz_grade."\" ></td>";
                        }
                        if($section==1)
                        {
                        ...
                        if($participate==1)
                        {
                            echo "<td ><input ".((isset($grades->semester_id) && $grades->semester_id != $AvilableSemester )?' disabled':'')." style=\"text-align:center;\" name='participate' ".((isset($grades->participation))?'value='.$grades->participation.'':'value="0"')." type=\"number\" min=\"0\" max=\"".$grade_dist->participation."\"></td>";
                        }
                        if($attendance==1)
                        {
                            echo "<td ><input ".((isset($grades->semester_id) && $grades->semester_id != $AvilableSemester )?' disabled':'')." style=\"text-align:center;\" name='attendance' ".((isset($grades->attendance))?'value='.$grades->attendance.'':'value="0"')." type=\"number\" min=\"0\" max=\"".$grade_dist->attendance."\"></td>";
                        }
                        if($project==1)
                        {
                            echo "<td ><input ".((isset($grades->semester_id) && $grades->semester_id != $AvilableSemester )?' disabled':'')." style=\"text-align:center;\" name='project' ".((isset($grades->project))?'value='.$grades->project.'':'value="0"')." type=\"number\" min=\"0\" max=\"".$grade_dist->project."\"></td>";
                        }
                        if($assigment==1)
                        {
                            echo "<td ><input ".((isset($grades->semester_id) && $grades->semester_id != $AvilableSemester )?' disabled':'')." style=\"text-align:center;\" name='assigment' ".((isset($grades->assignment))?'value='.$grades->assignment.'':'value="0"')." type=\"number\" min=\"0\" max=\"".$grade_dist->assignment."\"></td>";
                        }
                    @endphp
    ...
    <div class="col-12 text-center">
        <a href="/exportStudent_Doc/{{Request::input('courseID') }}/{{ Request::input('SemesterName') }}"><button class="btn btn-outline-secondary">create Excel Sheet &nbsp;<i class="far fa-file-excel"></i></button></a>
    </div>
    {{---------------------------------------MailBox------------------------------------------------------}}
    <div class="composeMessage" style="padding: 15px; left: 27%; display: none; width: auto; height: auto;">
        <div class=""><div class="" style="overflow: auto; width: 605px; height: 400px;">
                <div id="fancy2" style="display: block;">
                    ...
                    <form action="/Panel/Doctor/NotifyStudents" method="POST">
                        @csrf
                        @php
...
@section('scripts')
    ...
    <script src=" {{ asset('/js/frontend/Doc_ManageCourse.js') }}"></script>
@endsection
